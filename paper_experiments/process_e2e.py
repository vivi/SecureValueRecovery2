    ...
    with open(file_path, "r") as csv_file:
        ...
        for row in rows[1:]:
            try:
                ...
                measurements["create shares"].append(float(row[3]) / 1e3)
                measurements["create shares %"].append(float(row[3]) / 1e3 / total)
...
print("AVERAGE CREATE SHARES %", np.mean(measurements["create shares %"]))
...
with open(out_file, "w") as f:
    f.write("i,prepare OPRFs,call servers,finalize OPRFs,create shares,total\n")
    for i, total in enumerate(measurements["total"]):
        a = measurements["prepare OPRFs"][i] / total * 100
        b = measurements["call servers"][i] / total * 100 + a
        c = measurements["finalize OPRFs"][i] / total * 100 + b
        d = measurements["create shares"][i] / total * 100 + c
        e = total / total * 100
